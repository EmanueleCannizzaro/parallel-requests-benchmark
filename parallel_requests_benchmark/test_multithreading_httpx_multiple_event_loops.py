...
logger = logging.getLogger('parallel_requests_benchmark.test_multithreading_multiple_event_loops')
...
def main():
    ...
    def create_and_run_event_loop(task_count):
        try:
    ...
    with ThreadPoolExecutor(max_workers=config.NO_OF_THREADS) as executor:
        futures = [executor.submit(create_and_run_event_loop, tasks_per_thread) for _ in range(config.NO_OF_THREADS)]
